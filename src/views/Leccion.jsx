...
const AnimacionEscudo = ({ onFin }) => {
  useEffect(() => {
    vibrar(30); // vibración al activarse el escudo
    confetti({
...
// ══════════════════════════════════════════════════════════════════════════
// PANTALLA DE VICTORIA (MEJORADA VISUALMENTE)
// ══════════════════════════════════════════════════════════════════════════
const PantallaVictoria = ({
  ...
  useEffect(() => {
    // Vibración de celebración al completar nivel
    vibrar([100, 100, 200]);
      ...
      <div className="w-full max-w-sm space-y-5 animate-slide-up z-10">
        ...
        {escudosUsados > 0 && (
          <div className="glass-card rounded-2xl p-4 border border-yellow-400/20 flex items-center gap-3">
            ...
            <div><p className="text-yellow-400 font-black text-sm">Escudo de San Miguel</p><p className="text-white/40 text-xs">Absorbió {escudosUsados} error{escudosUsados > 1 ? 'es' : ''}</p></div>
          </div>
        )}
...
// ══════════════════════════════════════════════════════════════════════════
// EJERCICIOS (sin cambios funcionales, solo mantienen la lógica)
// ══════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════
// COMPONENTE PRINCIPAL DE LECCIÓN (CON VIBRACIONES Y MEJORAS UX)
// ══════════════════════════════════════════════════════════════════════════
...
const Leccion = () => {
  ...
  const tiposPorVerso = useMemo(() => {
    ...
    for (let i = 0; i < banco.length; i++) { const nuevo = elegirTipo(anterior); tipos.push(nuevo); anterior = nuevo; }
    return tipos;
  }, [oracionActual?.id]);
  ...
  if (!oracionActual?.versos?.length) { setEnLeccion(false); return null; }
  ...
  const validar = async () => {
    ...
    if (correcto) {
      vibrar([20]); // vibración suave al acertar
      ...
      if (escudoActivo) {
        vibrar([30]); // vibración al activar escudo
        ...
        vibrar([50, 50]); // vibración doble para error
        setResultado('error');
  ...
  if (mostrarVictoria) return <PantallaVictoria oracion={oracionActual} monedasGanadas={monedasAc} errores={errores} totalVersos={banco.length} escudosUsados={escudosUsados} onVolverMapa={volverAlMapa} />;
