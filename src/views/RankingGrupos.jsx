...
const RankingGrupos = () => {
  ...
  if (cargando) {
    return (
      <div className="flex flex-col items-center gap-3 py-12 animate-pulse">
  ...
  if (error) {
    return (
      <div className="glass-card rounded-2xl p-6 text-center border border-red-500/20 animate-slide-up">
  ...
  return (
    <div className="py-6 space-y-6 animate-slide-up">
      ...
      <div className="text-center mb-2">
        <p className="text-[9px] font-black text-yellow-400 uppercase tracking-[0.5em] animate-pulse">Competencia entre grupos</p>
        <h2 className="text-3xl font-black text-white tracking-tighter mt-2 bg-gradient-to-r from-white to-yellow-200 bg-clip-text text-transparent">
          Ranking de Grupos
        </h2>
      ...
      {grupos.map((grupo, idx) => {
        ...
        return (
          <div
            key={grupo.nombre}
            className={`glass-card rounded-3xl border bg-gradient-to-b ${estilo.bg} ${estilo.border} overflow-hidden transition-all duration-300 hover:shadow-xl hover:scale-[1.01] animate-fade-in`}
          ...
          >
            ...
            <div className="p-5 pb-3 border-b border-white/10">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-3">
                  <div className="text-4xl drop-shadow-lg">{medalla}</div>
                  <div>
                    <h3 className="text-2xl font-black text-white tracking-tighter">{grupo.nombre}</h3>
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      <span className="text-yellow-400 font-black text-sm">🏆 {grupo.monedasTotal.toLocaleString()} monedas</span>
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-full bg-white/10 text-white/80 font-black text-[10px] uppercase tracking-wider hover:bg-white/20 transition-all duration-300 flex items-center gap-1 hover:scale-105 active:scale-95"
                >
                  {estaExpandido ? (
                    <>
                      <span>▲</span> Ver menos
            ...
            <div className="p-4 space-y-2">
              {topAmostrar.map((usuario, i) => {
                ...
                // 🔥 CORRECCIÓN CLAVE: Ahora detecta rutas locales (/images/...), http, data:image, o cualquier ruta que empiece con '/'
                const esImagen = avatar?.startsWith('data:image') || 
                                 avatar?.startsWith('http') ||
                                 avatar?.startsWith('/');  // Esto incluye /images/ y / cualquier ruta absoluta

                return (
                  <div
                    key={usuario.uid}
                    className="flex items-center gap-3 p-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-300 group hover:scale-[1.01]"
                  >
                    ...
                    <div className="w-9 h-9 rounded-full bg-white/10 border border-white/20 flex items-center justify-center overflow-hidden shadow-inner">
                      {esImagen ? (
                        <img 
                          src={avatar} 
                          alt={usuario.nombre} 
                    ...
                    <div className="flex-1">
                      <p className="font-black text-sm text-white truncate group-hover:text-yellow-300 transition-colors">
                        {usuario.nombre}
                      </p>
                      <p className="text-white/30 text-[9px] font-bold">
                        Nivel {usuario.nivelActual} · Racha {usuario.racha}🔥
                      </p>
                    </div>
                    <div className="flex items-center gap-1 bg-black/20 px-2 py-1 rounded-full">
                      <span className="text-sm">🪙</span>
                      <span className="font-black text-sm text-yellow-400">{usuario.monedas.toLocaleString()}</span>
                    </div>
                  </div>
                );
              })}
              {!estaExpandido && grupo.estudiantes.length > 3 && (
                <button
                  onClick={() => toggleExpandir(grupo.nombre)}
                  className="w-full text-center py-2 text-white/40 text-[10px] font-black uppercase tracking-wider hover:text-yellow-400 transition-colors duration-300 hover:scale-105"
                >
